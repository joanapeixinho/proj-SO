...
char *parse_message(uint8_t opcode, char *pipename, char *box_name) {
    ...
    char *buffer = (char *)malloc(
        sizeof(uint8_t) + CLIENT_NAMED_PIPE_PATH_LENGTH + BOX_NAME_LENGTH);
    ...
    // if pipename length is bigger than CLIENT_NAMED_PIPE_PATH_LENGTH, it will
    // be truncated
    ...
    if (box_name != NULL) {
        // int bar = '/';
        // if box_name length is bigger than BOX_NAME_LENGTH, it will be
        // truncated memcpy(tmp, &bar, sizeof(char));
