...
int pcq_enqueue(pc_queue_t *queue, void *elem){
    ...
    while(queue->pcq_current_size == queue->pcq_capacity){
        if(pthread_cond_wait(&queue->pcq_pusher_condvar, &queue->pcq_pusher_condvar_lock) == -1){
...
void *pcq_dequeue(pc_queue_t *queue){
    ...
    while(queue->pcq_current_size == 0){
        if(pthread_cond_wait(&queue->pcq_popper_condvar, &queue->pcq_popper_condvar_lock) == -1){
            pthread_mutex_unlock(&queue->pcq_popper_condvar_lock);
            return NULL;
        }
    }
