...
int main(int argc, char **argv) {
    ...
    if (strcmp(command, "create") == 0 || strcmp(command, "remove") == 0) {
        ...
        write_pipe(register_pipefd, buffer,
                   sizeof(uint8_t) +
        ...
        write_pipe(register_pipefd, buffer,
                   sizeof(char) + CLIENT_NAMED_PIPE_PATH_LENGTH * sizeof(char));
        pipefd = open(pipe_name, O_RDONLY);
        if (pipefd < 0) {
            fprintf(stderr, "Error opening pipe %s for reading\n", pipe_name);
            return -1;
...
int list_boxes(int pipefd) {
    ...
    while (last == 0) {
        ...
        if (op_code != OP_CODE_LIST_BOXES_ANSWER) {
            fprintf(stderr,
                    "ERROR: Unexpected operation code %d in list_boxes\n",
                    op_code);
            return -1;
        }
        read_pipe(pipefd, &last, sizeof(uint8_t));

        read_pipe(pipefd, boxes[count].box_name,
                  BOX_NAME_LENGTH * sizeof(char));
